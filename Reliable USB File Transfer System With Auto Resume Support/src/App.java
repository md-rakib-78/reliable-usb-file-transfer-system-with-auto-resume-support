...
public class App extends JFrame {
    ...
    private JPanel buildFolderCircleBtn(String label, Color bgColor, String actionId) {
        ...
        // Logo icon fetch
        ImageIcon icon = new ImageIcon(getClass().getResource("/assets/icon/src_folder_icon.png"));
        ...
        circle.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                onFolderButtonClicked(actionId);
            }

            @Override
            public void mouseEntered(MouseEvent e) {
                circle.setBackground(bgColor.darker());
            }
    ...
    // ── History table with file transfer records ───────────────────────────────────────────────
    ...
    // Transfer Status cell as a colored button label
    ...
    private void onFolderButtonClicked(String which) {
        ...
        if (which.toUpperCase().equals("SRC")) {
            ...
            if (transferAttr.sourcePath != null) {
                ...
                JLabel label = new JLabel("<html><center>Source file already selected do you want to choose another?</center></html>");
            ...
            if (transferAttr.sourcePath == null) {
                ...
                JOptionPane optionPane = new JOptionPane("No Source file selected.",JOptionPane.PLAIN_MESSAGE);

                // Create dialog from option pane
                JDialog dialog = optionPane.createDialog(this, "Message");

                // Set ONLY dialog title bar icon
                dialog.setIconImage(scaledImg);
        ...
        if (which.toUpperCase().equals("DEST")) {
            ...
            if(transferAttr.sourcePath == null) {
                ...
                Image img = icon.getImage();
                Image scaledImg = img.getScaledInstance(35, 35, Image.SCALE_SMOOTH);
                JOptionPane optionPane = new JOptionPane("Please select source file first.",JOptionPane.WARNING_MESSAGE);
                JDialog dialog = optionPane.createDialog(this, "Message");
                dialog.setIconImage(scaledImg);
                dialog.setVisible(true);
            ...
            if (transferAttr.destFolder == null) {
                ...
                JOptionPane optionPane = new JOptionPane("No destination folder selected.",JOptionPane.PLAIN_MESSAGE);
            ...
            String fileExtension = transferAttr.sourceFileName.contains(".")
                    ? transferAttr.sourceFileName.substring(transferAttr.sourceFileName.lastIndexOf(".") + 1)
                    : "";
    ...
    private void onStatusButtonClicked(int row, String status) {
        ...
        if (status.equalsIgnoreCase("Completed")) {
            ...
            new CompleteTransferStatus(row + 1);
            ...
            for (jsonAttribute tr : all) {
                if (row == Integer.parseInt(tr.si) - 1 && tr.transferStatus.equalsIgnoreCase("Interrupted")) {
                    ...
                    new InterruptedTransferStatus(row + 1, tr.fileName, tr.sourcePath, tr.destinationPath, tr.transferId);
                    dispose();
                    System.out.println("Done");
